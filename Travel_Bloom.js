...
function showPage(pageId) {
  ...
  function classifyKeyword(keyword) {
    ...
    if (kw === 'country' || kw === 'country') return 'country';
  ...
  function renderCards(places, label) {
    ...
    if (!places || places.length === 0) {
      ...
      subText.textContent = 'Try searching for "beach", "temple","contry" or a country name like "Japan".';
  ...
  async function handleSearch() {
    ...
    if (category === 'beach') {
      ...
      // city search (look for matching country name or city name)
      label = 'Destinations';
